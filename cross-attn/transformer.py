...
class CrossAttention(nn.Module):
    ...
    def forward(self, target, source, attn_mask=None):
        # target: [B, L, D]
        # source: [B, S, D]
        # attn_mask: [B, L, S]
        q = self.queries(target)
        k = self.keys(source)
        v = self.values(source)
        # [B, L, H]
...
class CrossTransformerBlock(nn.Module):
    def __init__(self, dim, h_dim):
        super().__init__()
        self.self_attn = Attention(dim, h_dim)
        self.cross_attn = CrossAttention(dim, h_dim)
        self.ffn = FeedForward(dim)
    ...
    def forward(self, target, source, attn_mask=None):
        target_proc = self.self_attn(target, attn_mask=attn_mask)
        target = self.norm1(target_proc + target)
        x = self.cross_attn(target, source)
        x = self.norm2(x + target)
        y = self.ffn(x)
        return self.norm3(y + x)
...
class CrossTransformer(nn.Module):
    def __init__(self, dim, h_dim, n_layers):
        super().__init__()
        self.blocks = nn.ModuleList(
            [CrossTransformerBlock(dim, h_dim) for _ in range(n_layers)]
        )

    def forward(self, target, source, attn_mask=None):
        for block in self.blocks:
            target = block(target, source, attn_mask=attn_mask)
        return target
